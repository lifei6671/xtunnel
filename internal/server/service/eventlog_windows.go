...
func ensureWindowsEventSource(store windowsEventSourceStore) (created bool, err error) {
	...
	if exists {
		if !managed {
			return false, errors.New("refusing to overwrite an unmanaged or modified XTunnelServer Event Log Source")
		}
		return false, nil
	}
...
func removeManagedWindowsEventSource(store windowsEventSourceStore) error {
	...
	if !managed {
		return errors.New("refusing to remove an unmanaged or modified XTunnelServer Event Log Source")
	}
...
func requireManagedWindowsEventSource(store windowsEventSourceStore) error {
	...
	if !exists {
		return errors.New("managed XTunnelServer Event Log Source is missing")
	}
	if !managed {
		return errors.New("XTunnelServer Event Log Source is unmanaged or modified")
	}
	return nil
}
