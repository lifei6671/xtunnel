...
XTunnel Standalone V0.1 正在按开发计划逐步实现。当前 Server/Agent 已具备配置加载和前台进程生命周期骨架，尚未启动 Management、Ingress、Agent Gateway 或数据库。
...
当前进程在配置校验通过后保持前台运行，收到 `SIGINT` 或 `SIGTERM` 后正常退出。完整启动链、监听器、数据库和 Drain 流程将在后续任务中接入。
