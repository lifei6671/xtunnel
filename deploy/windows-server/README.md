...
普通停止要求成功退出。刻意保持活动连接直到排空期限的场景保留现有
`DeadlineExceeded` 非零退出语义：前台核对独立进程的完整错误输出，只接受期限到达及
TLS `closeNotify` 超时后底层连接已关闭的限定错误；SCM 核对服务特定退出码、进程退出、
停止时序与重启业务，并结合运行回调截止错误传播的单元测试。SCM 的通用
