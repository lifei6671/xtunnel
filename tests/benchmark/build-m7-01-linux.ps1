...
try {
    ...
    if (-not $AllowDirty -and $worktreeStatus.Count -gt 0) {
        throw 'A formal Linux benchmark build requires a clean worktree. Use -AllowDirty only for development smoke.'
    }
    ...
    $targets = @(
        @{ Package = './internal/proxy'; Name = 'proxy.test' },
        @{ Package = './internal/tunnel'; Name = 'tunnel.test' },
        @{ Package = './internal/server/httpingress'; Name = 'httpingress.test' }
    )
    foreach ($target in $targets) {
        $outputPath = Join-Path $resolvedOutput $target.Name
        & go test -c -o $outputPath $target.Package
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to build Linux benchmark binary for $($target.Package)."
        }
    }
    ...
    $proxyHash = (Get-FileHash -Algorithm SHA256 (Join-Path $resolvedOutput 'proxy.test')).Hash.ToLowerInvariant()
    $tunnelHash = (Get-FileHash -Algorithm SHA256 (Join-Path $resolvedOutput 'tunnel.test')).Hash.ToLowerInvariant()
    $httpIngressHash = (Get-FileHash -Algorithm SHA256 (Join-Path $resolvedOutput 'httpingress.test')).Hash.ToLowerInvariant()
    $manifestLines = @(
        ...
        "tunnel_sha256=$tunnelHash"
        "httpingress_sha256=$httpIngressHash"
    )
    ...
    Write-Host "M7-01 Linux benchmark binaries: $resolvedOutput"
}
finally {
