...
- Go 任务的证据必须记录 `go env GOVERSION` 和 `go env GOTOOLCHAIN`；实际工具链必须是稳定的 `go1.27.1+`，且 `GOTOOLCHAIN=local`。
...
M0-09 部署/包装验收 ──→ M0-12 完整 M0 Gate ──→ M7 Alpha Gate ──→ M8-01 至 M8-05
```
...
- M8 是 Alpha Gate 完成后的 Windows Server 范围扩展；M8-01 冻结契约与构建基线，M8-02 完成 Stable Target/Lock/文件身份，M8-03 至 M8-05 串行完成安全基线、SCM Runtime 与 amd64 Preview Gate。arm64 仅保持交叉构建兼容性；Windows 的在线维护、自动 Restore、事务升级回滚和自管理删除不属于当前 M8。
...
| ID | 任务 | 依赖 | 产物 | 验收要点 | 状态 |
| --- | --- | --- | --- | --- | --- |
| M0-01 | 建立 Go Module 与目录骨架 | 无 | `go.mod`、`cmd/server`、`cmd/agent`、`internal/*` 骨架 | `go.mod` 声明 `go 1.27` 并由 `toolchain` 记录 `go1.27.1` 下限；提供只接受稳定 `1.27.x` 与 `GOTOOLCHAIN=local` 的版本检查入口；`go test ./...`、`go vet ./...`；无空壳公共抽象 | `DONE` |
...
| M0-10 | CI 和跨平台构建矩阵 | M0-02至 M0-08 | CI Workflow | CI/OCI Builder 使用最新稳定 `1.27.x` 补丁并设置 `GOTOOLCHAIN=local`；干净 checkout 中 Proto/Web/Go 顺序构建；Linux amd64/arm64 进程 Smoke。M0-09 的 Compose Runtime Smoke 仍由 M0-09 单独验收 | `DONE` |
...
- [x] 根 `go.mod` 声明 `go 1.27`，根/工具 Module 记录 `go1.27.1` 下限，CI 和 OCI Builder 使用最新稳定 `1.27.x` 补丁；验收设置 `GOTOOLCHAIN=local`，并记录实际的 `go env GOVERSION`/`GOTOOLCHAIN`。
- [x] `go test ./...` 通过。
- [x] `go vet ./...` 通过。
...
- [x] M8-01 契约、Config 对 Service SID 只读且 Data/Runtime 为 Modify 的 Protected DACL、Windows 原生加载的配置示例和 Windows amd64/arm64 构建基线通过；arm64 不被描述为运行或发布支持。
- [x] Stable Target、External Lock、Volume/File Identity、大小写与 Reparse Point 防线在 Windows amd64 原生测试通过，第二进程在触碰 SQLite、Key 或 Listener 前快速失败。
- [x] Protected DACL、Pinned Private Key/Token Master Key 耐久发布、Public TLS 的可信 Owner 与有效安全性质、Service SID 读取和非可信主体的写入/删除/ACL/owner/私钥读取拒绝均通过；不接管外部证书管理器 ACL，未知状态 fail-closed。
...
M0、M0.5、M1、M2、M3、M4、M5、M6 与 M7 已全部完成；历史完成数保持 95，加入收敛后的 M8 后全局完成数为 `98/100`，M8 为 `3/5 IN_PROGRESS`。M8-01、M8-02 已闭环，M8-03 阶段验收已通过；既有 Alpha Release Gate 不覆盖 Windows Server，M8 Windows Server amd64 Preview Gate 七项保持未勾选。
...
| Post-Alpha Windows Server 平台与发布契约 | M8-01 完成前冻结；M8-05 完成前不得宣称正式支持 | 用户已授权将剩余 M8 收敛为五阶段基础支持范围。Windows Server Preview 目标为 amd64、NTFS、`XTunnelServer`、LocalService + Service SID、ProgramFiles Binary、ProgramData Config/Data/Runtime、Protected DACL、Stable Target External Lock、Windows 文件身份、基础 SCM Runtime 与 amd64 原生 Preview Gate；arm64 只保持交叉构建兼容性。在线 Backup、自动 Restore、Named Pipe、SCM 事务升级回滚、运行中 EXE 自替换和自卸载不属于当前 M8；Windows 不进入 OCI 平台集合。任何局部构建或单架构 Smoke 不改变用户可见支持矩阵。 |
...
## 2026-09-03 · M8-01 Windows Server 契约、配置示例与双架构构建基线 · REVIEW

- 提交与产物：实现提交 `b99d49fcc9dcfb57bba48e57b2c51311f3b62922` 新增 Windows Server 完整配置示例、Schema 同步测试、Post-Alpha M8 冻结契约和 Windows amd64/arm64 CI Build Floor；CI 修复提交 `856e3418a3b30fab6496c3273af31872f1b082a4` 将 M8-02 才验收的 Data Target Runtime 测试保持为双架构编译证据，并让 Windows PowerShell 在保留退出码检查的前提下读取 CLI stderr 帮助输出。
- 精确 CI：实现提交的 [Push CI #33704844143](https://github.com/lifei6671/xtunnel/actions/runs/33704844143) 暴露上述两个 Build Floor 边界并返回失败；修复提交的 [Push CI #33706041140](https://github.com/lifei6671/xtunnel/actions/runs/33706041140) 为 `completed / success`。最终 Run 完成 Windows amd64 Server Test/Vet、arm64 原生配置加载测试、双架构 Server 测试二进制编译、Binary 构建、CLI 帮助与 Go 1.27.0 工具链读回，两个 Linux Verify 与两个特权 Chaos Job 也全部成功；Alpha Release-only Job 在普通 Push 按设计跳过。
- 复审：六文件分区复审与最终集成复审均完成；修复后的 `.github/workflows/ci.yml` 由独立 `CHILD_AGENT` 重新审查，Verdict=`APPROVED`、Review Gate=`PASSED`、Coverage=`COMPLETE`、Freshness=`FRESH`、P0/P1/P2/P3=`0/0/0/0`。Windows Public TLS operator-owned Cert/Key 的文件身份与权限边界、Binary/Config/Data/Runtime 精确 DACL，以及 M8-01/M8-02/M8-06 证据隔离均已复核。
- 状态影响：M8-01 从 `IN_PROGRESS` 进入 `REVIEW`，全局 `DONE` 保持 `95/101`，M8 为 `0/6 REVIEW`；M8-02 至 M8-06 保持 `NOT_STARTED`，M8 Release Gate 八项保持未勾选。本次未勾选任何产品任务，等待用户明确阶段复审后再决定是否转为 `DONE` 并解锁 M8-02。
- 文档同步：只更新开发计划的当前阶段、任务状态与精确证据。根 README、总技术方案、配置示例、Schema、Proto、OpenAPI、Migration、依赖、生产部署资产、权限实现、日志契约和 `docs/static` 图片无需再次修改，因为本轮只回写已提交的构建基线证据，没有新增用户可见运行支持。

## 2026-09-03 · M8-01 用户阶段复审与 M8-02 启动 · DONE

- 阶段批准：用户明确确认 M8-01 阶段复审通过。批准范围包含实现提交 `b99d49fcc9dcfb57bba48e57b2c51311f3b62922`、CI 修复提交 `856e3418a3b30fab6496c3273af31872f1b082a4`、证据提交 `b1d5aebcb4178d8c2370e0f8cc7373b442256174` 和最终修正 Head `fcdc701f03c260b2f61b4f18d4cb98ad916ac6fa`；最终 [Push CI #33709519721](https://github.com/lifei6671/xtunnel/actions/runs/33709519721) 为 `completed / success`，六个正式 Job 全部成功。
- 复审与状态：最终 commit-bound 契约/Schema/证据分区和 Git/CI/保护面分区均为 `APPROVED`，Coverage=`COMPLETE`、Freshness=`FRESH`、P0/P1/P2/P3=`0/0/0/0`。M8-01 从 `REVIEW` 转为 `DONE`，M8 从 `0/6 REVIEW` 更新为 `1/6 IN_PROGRESS`，全局从 `95/101` 更新为 `96/101`；M8-02 依赖闭环并进入 `IN_PROGRESS`。
- M8-02 首个实现单元：先完成 Windows Data Target Resolver、Volume/File Identity、大小写不敏感 leaf 规范化与 Reparse Point/身份替换拒绝，再接入 External Lock。DACL、Durable Publisher、Restore、Named Pipe 与 SCM 继续由 M8-03 至 M8-05 按依赖串行实施。
- Gate 与发布边界：M8 Windows Server Release Gate 八项保持未勾选。根 README、总技术方案、配置示例、Schema、Proto、OpenAPI、Migration、依赖、CI/CD、部署资产、日志契约和 `docs/static` 图片无需修改；M8-06 完整原生运行、发布候选、聚合 Gate 与用户阶段签核闭环前，Windows Server 支持矩阵保持不变。

## 2026-09-03 · M8-02 Stable Target、External Lock 与 Windows 文件身份 · REVIEW

- 提交与产物：实现提交 `27e6aff7a558784072bae38afaeb3a49ebebdfe9` 完成 Windows Data Target Resolver、完整 `FILE_ID_INFO` 身份、Stable Target Hash、父目录生命周期固定、External Lock 及 Server/Backup/Restore/Gateway Bootstrap 顺序接入；修复提交 `094f0c160f9793a0aa773ecd0c9f5661d47371a6` 以 Handle 权威路径接受同一目标的 Windows 8.3 别名；测试修复提交 `3bdaed22ad22080a9b5668533bcf3447f032c9e1` 让断言使用同一权威规范路径。
- 安全与生命周期：Data Target 只接受绝对本地固定卷路径，逐级 no-follow 拒绝 UNC、设备路径、ADS、跨卷、Reparse Point、非法 leaf 与父目录身份替换；Target Hash 使用 64-bit Volume Serial、完整 128-bit File ID 和大小写不敏感 leaf。Parent Guard 在持久化资源完整生命周期内固定父目录并阻止父目录及祖先重命名，同时允许 Restore 替换 leaf；External Lock 位于 Known Folder API 解析的受管 Runtime 目录，使用 target-hash 命名、`LockFileEx` 非阻塞独占锁、Handle 固定及逆序关闭，竞争进程在触碰 Restore Journal、SQLite、Token Key 或 Listener 前失败，正常关闭与进程终止后均释放。
- 精确 CI：[Push CI #33717509091](https://github.com/lifei6671/xtunnel/actions/runs/33717509091) 暴露 8.3 短路径与 Handle 长规范路径的别名差异；[Push CI #33721914618](https://github.com/lifei6671/xtunnel/actions/runs/33721914618) 暴露测试仍按输入字符串断言规范路径。最终 Head `3bdaed22ad22080a9b5668533bcf3447f032c9e1` 的 [Push CI #33722274859](https://github.com/lifei6671/xtunnel/actions/runs/33722274859) 为 `completed / success`，Windows amd64 Agent Service、Windows arm64 Agent Runtime、Linux amd64/arm64 Verify 与双架构 Privileged Network Chaos 六个正式 Job 全部成功；Windows amd64 原生 Data Target/External Lock Test、Race、Vet、Server Build，以及 Windows arm64 原生 Data Target/External Lock/Bootstrap 顺序测试和 Server Build/读回均通过。四个 Alpha Release-only 执行 Job 与 `Alpha release gate` 聚合 Job 在普通 Push 按设计跳过。
- 本地验证：仓库固定 `go1.27.0` Docker Runner 中，`.\scripts\test.ps1 ./internal/server/datadir ./internal/server/durableops ./internal/server/bootstrap` 与后续 `.\scripts\test.ps1 ./internal/server/datadir` 均通过。`.\scripts\verify.ps1` 的本地完整复验因 Docker Desktop Named Volume 未能创建可用的 npm `.bin/tsc` 链接而停在 Web Check，记为环境阻塞的 `NOT VERIFIED`，不作为产品失败或正式通过证据；最终正式验收以成功的精确 CI 为准。
- 测试与复审：测试覆盖不创建目标 leaf、危险路径拒绝、8.3 别名、完整 128-bit File ID、父目录替换与新 Hash、父目录/祖先重命名阻断、受限 DACL、leaf 替换、正常/进程终止释放、Junction 拒绝、锁竞争/崩溃释放/Runtime 与 Lock Handle 固定，以及预置畸形 Restore Journal 下锁前失败且 Journal 字节与文件身份不变、DB/Key 不生成。最终独立全范围复审结论为 `APPROVED`，Review Gate=`PASSED`、Coverage=`COMPLETE`、Freshness=`FRESH`、P0/P1/P2/P3=`0/0/0/0`。
- 状态与边界：M8-02 从 `IN_PROGRESS` 进入 `REVIEW`，全局 `DONE` 保持 `96/101`，M8 为 `1/6 REVIEW`；M8-03 至 M8-06 保持 `NOT_STARTED`，M8 Release Gate 八项保持未勾选。等待用户明确阶段复审后再决定是否将 M8-02 转为 `DONE` 并解锁 M8-03。
- 文档同步：仅更新本开发计划的状态与证据。根 README、总技术方案、配置示例、Schema、Proto、OpenAPI、Migration、依赖、生产部署资产、日志契约和 `docs/static` 图片保持不变；M8-06 完整原生运行、发布候选、聚合 Gate 与用户阶段签核闭环前，Windows Server 支持矩阵保持不变。

## 2026-09-03 · M8-02 用户阶段复审与 M8-03 启动 · DONE

- 阶段批准：用户明确回复“`M8-02 阶段复审通过`”。批准范围包含实现提交 `27e6aff7a558784072bae38afaeb3a49ebebdfe9`、Windows 8.3 规范路径修复提交 `094f0c160f9793a0aa773ecd0c9f5661d47371a6`、测试断言修复提交 `3bdaed22ad22080a9b5668533bcf3447f032c9e1`，以及证据提交 `4159e02939ace7a93387fd48b739e8228653c807`。
- 精确证据：最终实现 [Push CI #33722274859](https://github.com/lifei6671/xtunnel/actions/runs/33722274859) 与证据提交 [Push CI #33723985760](https://github.com/lifei6671/xtunnel/actions/runs/33723985760) 均为 `push / attempt 1 / completed / success`，各自六个常规正式 Job 全部成功；证据提交的最终 commit-bound 复审为 `APPROVED`、Coverage=`COMPLETE`、Freshness=`FRESH`、P0/P1/P2/P3=`0/0/0/0`，工作区、暂存区和保护面均通过复核。
- 状态影响：M8-02 从 `REVIEW` 转为 `DONE`，全局从 `96/101` 更新为 `97/101`，M8 从 `1/6 REVIEW` 更新为 `2/6 IN_PROGRESS`；M8-03 依赖闭环并进入 `IN_PROGRESS`，M8-04 至 M8-06 保持 `NOT_STARTED`。
...
## 2026-09-03 · Windows Server Scope Reset · IN_PROGRESS

- 范围重设：用户明确将剩余 M8 收敛为 Windows Server amd64 + NTFS 的基础支持/Preview。M8-01、M8-02 的 `DONE` 证据保持原义；M8-03 改为 Windows Security Baseline，M8-04 改为 `XTunnelServer` SCM Runtime，M8-05 改为 Windows amd64 Preview Gate。任务总数从 101 调整为 100，已完成数保持 97，M8 为 `2/5 IN_PROGRESS`。
- 保留边界：LocalService + Service SID、ProgramData Protected DACL、Stable Target、External Lock、Reparse/对象身份防护、Secret 安全发布与 Public TLS 的可信 Owner + 有效安全性质校验仍是发布前置。SQLite Driver 继续负责数据库文件、WAL、SHM、journal 与临时文件的 Windows VFS 语义；XTunnel 不自定义 VFS。
- 维护边界：当前 M8 不实现在线 Admin/Backup Named Pipe、产品管理的在线 Backup、自动 Restore、Restore Journal 自动恢复、目录级事务切换、跨崩溃矩阵、SCM 事务升级回滚、运行中 EXE 自替换、自卸载或延迟删除。首个管理员创建在服务停止后通过离线 CLI 和 External Lock 完成；异常持久化状态保留现场并 fail-closed。
- 发布边界：arm64 仅保持交叉构建兼容性，不再进入 Windows Server 原生运行或 Release Gate。只有 M8-05 的 Windows amd64 Preview Gate、同候选完整 CI、独立复审和用户阶段签核闭环后，用户可见材料才可标注 Windows Server amd64 Preview。
- 本次仅重置未完成需求与 Gate，不改变已提交实现或其历史验证证据；本次未勾选任何产品任务。

## 2026-09-03 · Go 1.27 补丁跟随构建基线

- 策略：根/工具 Module 的 `toolchain` 下限更新为 `go1.27.1`；本地 Docker、OCI Builder 与 CI 在每次构建选择最新稳定 `1.27.x` 补丁，版本门禁仅接受形如 `go1.27.[1-9][0-9]*` 的稳定版本并保持 `GOTOOLCHAIN=local`。Go 1.28、预发布、开发版本与前导零补丁号均不构成可接受构建环境。每份验证证据继续记录实际 `GOVERSION`。
- 同步范围：版本检查入口、Docker Compose/OCI、CI 八个 Go Setup Job、OpenAPI Bootstrap、M7 运行器与预构建 Artifact Manifest 校验均已使用同一范围；所有 OCI build 入口显式 `--pull`，避免浮动标签复用陈旧本地镜像。
- 本地验证：Docker Desktop 拉取 `golang:1.27-bookworm` 后读回 `go1.27.1/local`，`./scripts/verify.ps1` 完成 Web Check/Build、全仓 Go Test/Vet/Build；本机 `go1.27.1/local` 的版本检查、Windows `winsecurity` 与 `durableops` 定向 Test/Vet、Server Build 均通过。`tools/test-openapi.sh` 在既有隔离目录被其自身 OpenAPI 命令清理后无法继续创建 fixture，记为 `BLOCKED`，不将其写为通过。
- 状态与 Gate：该构建基线变更不改变任何产品任务状态或 Gate；M8-03 仍为 `IN_PROGRESS`，本次未勾选任何产品任务。

## 2026-09-04 · M8-03 Windows 原生回归补全

- 测试夹具：仅为实际写入 pinned Gateway 身份的 Windows 测试创建受保护的临时前台目录；纯配置生命周期测试使用 `server.data_dir: auto` 并只经 fake storage 观察解析结果，不创建或写入用户 Profile。生产 DACL 与路径 Profile 策略未放宽。
...
## 2026-09-04 · M8-03 Windows Owner 原生证据补齐

- 当前候选：`683c8cbbf44a50bab119bf2426df719da7aa6f29`，已推送至 `origin/master`。增量测试与 CI 复审通过，原生产实现及其复审结论保持新鲜。
- [Windows amd64 Job #100917407440](https://github.com/lifei6671/xtunnel/actions/runs/33839053263/job/100917407440) 已成功。`go1.27.1/local` 的原生 Race 日志明确记录 `TestWindowsAcquireRejectsOwnerMismatchWithoutMutation/runtime` 与 `/lock` 均 `PASS`，时间分别为 `2026-09-04T05:11:01.9686558Z`、`05:11:01.9687278Z`；既有锁 DACL 的两个负向子用例同样通过。本机跳过的 Owner 运行证据现由当前提交的 CI 补齐。
- 当前提交的 Windows arm64 Agent runtime、双架构 privileged network chaos 也已成功；两项 Linux verify 仍在运行，尚不形成完整 CI 通过结论。M8-03 保持 `IN_PROGRESS`，本次未勾选产品任务或 Gate。

## 2026-09-04 · M8-03 精确 CI 与最终复审完成 · REVIEW

- 验收候选：`683c8cbbf44a50bab119bf2426df719da7aa6f29`。[CI #33839053263](https://github.com/lifei6671/xtunnel/actions/runs/33839053263) 为 `completed/success`；Linux amd64/arm64 verify、双架构 privileged network chaos、Windows Agent service、Windows arm64 Agent runtime 六个正式 Job 全部成功。五个条件跳过的 Alpha Release Job 不计为通过，也不扩大 Windows Preview Gate。
- Windows 原生验收：新增安全包测试、External Lock Race、既有锁 DACL 拒绝、Runtime/lock Owner 不符拒绝及 SCM/构建检查均有当前提交的 CI 证据。Owner 两个用例的实际 PASS 已经由独立复审者直接读取 Job 日志复核。
- 最终复审：`CHILD_AGENT PASSED`，Binding 为上述候选 SHA，已审范围 Coverage=`COMPLETE`、Freshness=`FRESH`；当前 Windows M8-03 受支持路径累计分区与启动链集成复审闭合，原 Runtime/lock P1 已解决。Linux V1 Restore 清理 P2 继续作为已记录的范围外后续项，不声明已修复。
- 状态：任务表、当前队列与仪表盘同步为 M8-03 `REVIEW`。实现、验证和独立代码复审已完成，等待用户明确阶段验收；M8-04、M8-05 仍为 `NOT_STARTED`，完成计数保持 `97/100`，本次不勾选产品任务或聚合 Gate。

## 2026-09-04 · M8-03 用户阶段验收 · DONE

- 用户明确确认：M8-03 阶段验收通过。验收实现为 `683c8cbbf44a50bab119bf2426df719da7aa6f29`，对应 [CI #33839053263](https://github.com/lifei6671/xtunnel/actions/runs/33839053263) 六个正式 Job 成功，独立复审 `PASSED`、Coverage=`COMPLETE`、Freshness=`FRESH`。详细命令与原生断言见前述证据记录。
- M8-03 更新为 `DONE`，全局完成计数 `98/100`，M8 完成 `3/5`；M8-04 的前置依赖已满足。M8-04、M8-05 保持 `NOT_STARTED`，Windows Preview Gate 仍按 M8-05 独立验收。

## 2026-09-04 · M8-04 实施准备与 Service Owner 权限决策

- 授权与基线：用户要求继续下一个任务，即 M8-04。起点为 `8f136a6e4939fdd653182188816e648bd569027f`，工作区干净；负责人为主代理，`scm_contract_reviewer` 完成独立只读契约审查。范围以方案第 191.1 至 191.5 节及 M8-04 任务行为准。
- 已核实实现：`internal/server/service/service.go` 的平台门禁仅接受 Linux，Windows 使用 unsupported 实现；`config.Load` 的校验固定调用前台 `pathprofile.Resolve`，SCM 不能仅替换进程入口；`bootstrap`、External Lock、Data Guard、Token Key 与 Gateway 文件安全策略需要保持同一个显式 Profile。现有 Agent SCM 可以参考生命周期与 Event Log 用法，但其重装替换、共享 LocalService ACL 和延迟删除行为不能直接作为 Server 安装契约。

### 待确认的权限契约补充

方案第 191.1 节目前要求精确三主体 DACL，Service SID 的 Modify 不包含修改 DACL/Owner。Windows 新对象 Owner 来自创建令牌，Service SID 本身不会改变默认 Owner；LocalService 创建的对象可能仍以 LocalService 为 Owner，而 Owner 隐含 `WRITE_DAC`。只检查三条 ACE 无法证明所要求的服务隔离。依据：[Windows 对象 Owner](https://learn.microsoft.com/en-us/windows/win32/secauthz/owner-of-a-new-object)、[Service SID 与 Owner](https://devblogs.microsoft.com/oldnewthing/20231004-00/?p=108849)、[OWNER RIGHTS](https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/understand-special-identities-groups#owner-rights)。独立审查确认这是权限模型决策，不是现有 M8-03 前台实现缺陷。

建议确认以下规则后再修改方案和实现：

1. 安装器创建的 Binary、Config、Data/Runtime 根目录固定 Owner 为 Builtin Administrators；既有安装对象只接受契约规定的可信 SYSTEM/Administrators Owner。运行时受管文件允许 LocalService Owner；离线管理员维护产生的对象使用 Administrators Owner，验证时按对象用途区分允许集合。
2. 保留 SYSTEM/Administrators Full Control、Service SID 的 Binary Read & Execute、Config Read、Data/Runtime Modify。在 Service Data/Runtime 及其受管文件的精确 DACL 中增加 `OWNER RIGHTS`（`S-1-3-4`）仅 `READ_CONTROL` 的限权 ACE，以抑制 Owner 隐含 `WRITE_DAC`；不给 LocalService 增加数据访问 ACE。目录向子对象传播该限制，XTunnel 自行发布的文件在创建时设置完整安全描述符并保持 Protected DACL。
3. SQLite Driver 创建的数据库及 sidecar 文件从 Data 目录继承限权规则，继续由 Driver 管理 Windows VFS；通过真实 Service Token 测试验证效果，不修改 VFS，也不靠启动后修 ACL 补救。

该补充改变第 191.1 节精确权限矩阵，按根 `AGENTS.md` 的权限模型变更规则等待用户确认。总方案暂不改写为已冻结的新规则。

### 实施分区与验收安排

1. **Profile 与安全原语**：显式贯通 Service 配置加载、固定路径、Data Guard、External Lock、Token Key 和 Gateway Secret 的策略选择；保留前台隔离。测试精确 Owner/DACL、服务首次创建及重启复用、宽泛权限/身份漂移拒绝、拒绝后现场不变、同为 LocalService 但不带 XTunnelServer SID 的负向访问，尤其验证 `WRITE_DAC`/`WRITE_OWNER` 被拒绝。
2. **安装与卸载**：在任何持久化修改前完成 amd64、提升权限、SCM、NTFS、配置、目标身份与 Marker 检查；发布固定 Binary/Config，建立 Service SID 与有限恢复策略。共享 XTunnel 祖先只验证可信 Owner、no-follow 和防替换安全性质，Server 精确权限矩阵限于其文件及私有子树。测试无权限、非法配置、非受管同名对象及 Reparse 拒绝无副作用；卸载保留 Config/Data 和其他组件对象。
3. **SCM 生命周期与离线维护**：先进入 Dispatcher，再加载固定配置并运行现有生命周期；Stop/Shutdown 派生取消，30 秒排空上界后主动关闭 Socket 并等待 owner；Event Log 写入失败和回调失败返回非零退出。按第 191.4 节已有要求，为固定配置下的提升管理员离线维护核验受管 SCM 已停止，再取得同一 External Lock；普通前台启动仍只接受前台 Profile。
4. **验证与复审**：原生相关 package Test/Vet 与锁、生命周期 Race；仓库 `./scripts/verify.ps1` 完成 Docker Desktop Linux 回归；Windows amd64 隔离提升权限 Runner 执行真实 install/start/stop/non-crash recovery/uninstall、Event Log、跨 Service Token 权限与保留数据 Smoke；arm64 仅构建兼容性。按安全原语、安装器、生命周期分区独立复审并做集成复审，最终证据绑定同一提交的完整 CI。具体新增 Smoke 入口在实现后落盘并记录，不把计划命令当成既有产物。

- 当前验证：`GOTOOLCHAIN=local` 下 `./tools/check-go-version.ps1` 通过，工具链为 `go1.27.1`。本轮仅文档准备，未修改可执行产物，代码交付复审为 `Review not required`；独立契约审查已完成。实现测试、Docker 全量验证及 SCM Smoke 均为 `NOT_RUN`，尚无 M8-04 可执行候选。
- 状态与范围：M8-04 为 `BLOCKED`，解除条件是确认上述 Owner/OWNER RIGHTS 补充。M8-03 保持 `DONE`，全局 `98/100`、M8 `3/5`，M8-05 保持 `NOT_STARTED`。本次未勾选任何产品任务或聚合 Gate；未修改主机 ACL、SCM、Event Source、总方案、README 或配置示例。

## 2026-09-04 · M8-04 权限补充确认与实施启动

- 用户明确确认上一条 Owner/OWNER RIGHTS 方案。第 191.1 节同步 Owner 分层与限权 ACE，第 191.4 节明确固定配置的提升管理员离线入口。M8-04 转为 `IN_PROGRESS`，开始安全原语、安装器和生命周期并行实施；实现验证与最终独立复审待完成。

## 2026-09-04 · M8-04 SCM 实现与本地验证

- 产物：Windows amd64 Server 安装、卸载、Dispatcher、SCM Ready/Stop/Shutdown、有限失败恢复及独立 Event Source 已实现。安装采用固定路径、创建时完整安全描述符和非覆盖发布；卸载保留 Config/Data/Runtime。Service Profile 贯通配置、存储、External Lock、Token Key 和 Gateway；固定配置的离线管理员入口先验证受管 SCM 停止，再使用同一 External Lock。
- 权限：Data/Runtime 使用 Service SID Modify 与 OWNER RIGHTS READ_CONTROL，运行文件按用途接受 LocalService/SYSTEM/Administrators Owner。产品共享祖先拒绝非可信主体的删除、安全控制及可原地设置 Reparse Point 的 WRITE_DATA/WRITE_ATTRIBUTES；操作系统 KnownFolder 保留其系统管理边界。真实跨服务令牌测试覆盖 SQLite/WAL/SHM、凭据、Gateway Key 与锁文件，显式 CI 开关下不得跳过。
- 本机证据：`GOTOOLCHAIN=local`、`./tools/check-go-version.ps1` 确认 `go1.27.1/local`；`go test -count=1 -timeout 10m ./...`、`go vet ./...`、`go mod verify` 均 exit 0。bootstrap/service/externallock/winsecurity 四包 `go test -race -count=1 -timeout 180s` 通过；共享祖先权限修复后五个相关 Windows 包测试、winsecurity Vet 与 Race 再次通过。Server Windows arm64 交叉构建成功，只作为构建兼容证据。
- Linux 回归：Docker Desktop `./scripts/verify.ps1` exit 0，完成指定 Go 镜像拉取、工具链门禁、Web Check/Build 与 Linux 全仓 Test/Vet/Build。工作树差异检查通过。
...
## 2026-09-04 · M8-04 Service Token 身份筛选

- 候选 19e73ee77764867845dea17641602f7ad5a015fd 的 [CI #33850401912](https://github.com/lifei6671/xtunnel/actions/runs/33850401912) 已通过完整 Server SCM smoke：安装、就绪、停止、重启、有限 non-crash 恢复、Event Log、卸载及重装保留。Event RecordId 边界下严格三次启动与延后仍三次断言通过。
- TestServiceTokenIsolation 实际执行，XTunnelServer 本服务 Token 检查通过，另一 LocalService 服务被策略筛选错误接受而失败；其后 NTFS 拒绝访问断言尚未执行。当前 validateServiceStorageToken 在检查 LocalService 的精确 Service SID 之前以 IsElevated 直接允许维护入口，未同时要求 Administrators 身份。修复按既定身份契约区分 SYSTEM、提升且启用的 Administrators，以及携带启用且非 deny-only 的 XTunnelServer SID 的 LocalService。
- M8-04 保持 IN_PROGRESS，继续身份筛选回归、独立复审与精确候选 CI；尚不计跨服务隔离通过。
- 本地验证：Go 1.27.1/local 下 15 项身份矩阵、winsecurity/externallock/bootstrap/service 四包 Test/Vet、winsecurity/externallock Race 通过，Docker Desktop ./scripts/verify.ps1 exit 0。安全增量独立复审及最终集成均 PASSED / Tier3，0 findings；真实 Token 测试与 smoke 断言保持。

## 2026-09-04 · M8-04 原生 SCM 与跨服务隔离证据

- 提交 cfbe201c4c67192cc981357ad8a18b2546cb4b8a 对应 [CI #33851737928](https://github.com/lifei6671/xtunnel/actions/runs/33851737928)，[Windows Job #100955860669](https://github.com/lifei6671/xtunnel/actions/runs/33851737928/job/100955860669) 为 success，Go 1.27.1 / GOTOOLCHAIN=local。
- 正式 Server SCM smoke 实际通过：安装、业务就绪、停止、重启、两次有限 non-crash 恢复后停止、Event Log、卸载及重装保留 Config/Data/Runtime。真实 TestServiceTokenIsolation 实际 RUN/PASS：XTunnelServer 与 XTunnelServerSecurityProbe 两个 LocalService 服务均完成各自权限断言，本服务可读写受管对象，异名服务不能选择存储策略或读取对象；双方 WRITE_DAC/WRITE_OWNER 被拒绝，受保护根与继承子对象的完整 ACL/Owner 矩阵通过。
- 提交绑定独立复核 PASSED：安全实现/单测、Token 测试和 smoke 的 SHA256 与冻结复审一致，代码 Gate 保持有效。Windows arm64 和双架构网络混沌作业均 success；Linux 完整验证继续运行，M8-04 暂保持 IN_PROGRESS。

## 2026-09-04 · M8-04 阶段交付待验收

- 实现候选：cfbe201c4c67192cc981357ad8a18b2546cb4b8a。[CI #33851737928](https://github.com/lifei6671/xtunnel/actions/runs/33851737928) 已 completed / success，六项常规作业全部 success：Windows amd64、Windows arm64、Linux amd64/arm64 完整验证及双架构网络混沌。Release 专用作业按 workflow 条件 skipped，不作为本阶段新 Release 证据；真实浏览器 E2E 在 amd64 执行通过，arm64 按矩阵跳过。
- Windows amd64 完整 SCM smoke 与 TestServiceTokenIsolation 实际通过，证据见上一节。Linux 双架构 Test/Vet、Fuzz、完整资源泄漏、产品数据面、备份恢复重连、可观测性、流式传输、HTTPS ingress、OCI、Compose 和 systemd 对应验证通过。实现后的本地 Windows Test/Vet/Race 与 Docker Desktop verify 亦通过。
- 独立审查：继承权限、安全身份筛选、真实 SCM 测试、smoke 及契约文档分区覆盖完整；最终集成 PASSED / Tier3，无未解决 P0/P1，提交绑定核对通过。后续变更仅为本计划的状态与证据同步，未改变已验证可执行产物。
- 状态：M8-04 转 REVIEW，待用户明确阶段验收；M8-03 保持 DONE，M8-05 保持 NOT_STARTED，全局 98/100、M8 3/5。Windows amd64 Preview Gate 和正式支持声明仍由 M8-05 控制。

## 2026-09-04 · M8-05 验收准备检查

- 本轮按“继续下一步”完成只读准备与既有代码验证；M8-04 仍 REVIEW，M8-05 依赖的用户阶段验收尚未记录，因此不变更完成计数或开始下一阶段实现。
- 基线：d0609ef843acbda12229b476f84731350d9ef905；可执行内容与 cfbe201 相同。Windows 本机 Go 1.27.1 / GOTOOLCHAIN=local，go test -count=1 -timeout 180s ./... 与 go vet ./... 实际通过。这是当前 checkout 开发反馈，普通 SCM 测试的 skip 不替代已取得的隔离 CI 证据。
- 已有基础：M8-04 完整 SCM、权限继承、身份与异名服务隔离证据通过；现有 Windows CI 的 Server Test/Vet 采用有限包列表（.github/workflows/ci.yml:553），尚未接入全仓原生验收。现有 tests/integration/tcp_echo_test.go 可复用 TCP/Gateway 与重连验证，但其 /proc FD 观测不能用作 Windows 句柄收敛证据。
- 产品链路缺口：deploy/windows-server/smoke.ps1:127 的端口连接检查不能证明 Management 认证、HTTP 响应、WebSocket 帧、TCP 转发和 Agent Gateway 认证的完整链路。需补 Windows 真实前台/SCM 产品验证、第二进程抢锁、重启状态及有界退出断言，复用已有协议与客户端入口；Linux 专属 E2E 不计 Windows 通过。
- 候选产物缺口：现有 Windows CI 构建临时 EXE 并读取工具链，尚无第 191.6 节要求的 xtunnel-server-windows-amd64.exe 候选归档、版本/PE amd64/SHA-256/Secret 扫描的完整链。拟在现有 tests/release 下增加 Windows 候选验证入口并复用 secretscan；Linux OCI 平台集合保持现状。
- 拟实施顺序：先补原生产品与资源收敛验收，再补 Windows 候选 Artifact 校验与 CI 接入，最后取得精确候选完整 CI、分区及集成独审并同步 Preview 文档。涉及的 CI/构建验收入口调整按根 AGENTS.md 取得明确授权后执行；M8-05 完成前不发布新的平台支持声明。

## 2026-09-04 · M8-04 验收通过与 M8-05 开始

- 用户明确确认 M8-04 阶段验收通过，并授权上述 M8-05 范围、CI/构建验收入口调整、提交推送及跟进 CI。M8-04 转 DONE，M8-05 转 IN_PROGRESS，全局 99/100、M8 4/5。
- M8-05 交付基线为 d0609ef843acbda12229b476f84731350d9ef905，包含上一轮待提交的验收准备记录。并行分区为真实 Windows 产品 Gate、Windows 候选 Artifact 验证，以及 CI/文档集成；每个分区冻结后独立复审，最终核对同一候选 Binary、运行时版本和 SHA-256。

## 2026-09-04 · M8-05 合并开发验证

- 产品入口位于 `tests/windows-server-gate/`，候选 Artifact 校验位于 `tests/release/windowsverify/`；CI 在原生 Windows amd64 执行全仓 Test/Vet，随后运行双模式产品 Gate、既有 SCM smoke 与令牌隔离，并校验同一 Server 候选。
- 当前工作区 Go 1.27.1 / GOTOOLCHAIN=local 下 `go test -count=1 -timeout 180s ./...`、`go vet ./...` 通过；Docker Desktop `./scripts/verify.ps1` 完成 Web 安装/类型检查/构建及 Linux 全仓 Test/Vet/build，exit 0。Workflow YAML 和 PowerShell AST 解析通过。这些均为开发反馈，隔离主机 Gate 在普通测试中明确跳过。
- Artifact 分区及 CI/文档分区独立复审通过；产品和集成复审要求补齐首次安装失败清理，以及日志、Config、SCM Registry 的实际凭据扫描。修复、受影响复验和最终独审完成后再冻结提交，M8-05 保持 IN_PROGRESS；真实候选运行、Artifact 与精确 CI 为 NOT_RUN。

## 2026-09-04 · M8-05 候选冻结

- 产品 Gate 完成前台与 SCM 的 Management/CSRF、Token 签发与真实 Agent Gateway、HTTP/WebSocket/TCP 半关闭、重启持久状态、运行态拒绝、活动连接停止及 Listener/PID 收敛断言。30 秒生产排空、31 秒 Socket 观测上界和 35 秒最终进程收尾分别记录。
- 敏感信息检查覆盖本轮密码、Token、Cookie/CSRF、实际 Master Key 与冻结私钥等形状；在有界内存中检查进程/CLI 日志、候选 Binary、配置、SCM Registry 和服务事件，溢出或命中即失败。安装中途失败进入已登记清理，不完整且归属无法证明的现场明确报错并保留。
...
## 2026-09-04 · M8-05 候选验收完成，进入 REVIEW

- 实现候选为 `43808023fd3bff5b19243ac63ecaf9d3789ee40b`；[完整 CI #33868358816](https://github.com/lifei6671/xtunnel/actions/runs/33868358816) 六个常规作业全部成功，包含 Windows amd64/arm64、Linux amd64/arm64 完整 verify 与双架构网络混沌。Release-only 作业按普通 Push 条件跳过，不作为本次发布证据。
- [Windows amd64 Job #101008258057](https://github.com/lifei6671/xtunnel/actions/runs/33868358816/job/101008258057) 完成原生全包 Test/Vet、Server 安全定向 Race、双架构构建、双模式产品、既有 SCM smoke、真实 Service SID 隔离与候选 Artifact 验证，并确认工作树干净。
- `go test -v -count=1 -timeout 600s ./tests/windows-server-gate -run '^TestWindowsServerProductGate$'` 真实运行 128.82 秒通过；前台 63.63 秒、SCM 65.07 秒，覆盖管理、HTTP/WebSocket/TCP/半关闭、Agent Gateway、抢锁/维护拒绝、持久身份与服务恢复、活动停止、重启业务及清理后 Secret 扫描。活动 Socket/进程退出分别为前台 30.020/30.050 秒、SCM 30.001/30.030 秒。
- `TestServiceTokenIsolation` 实际运行 0.46 秒，XTunnelServer 与 SecurityProbe 的真实 SCM LocalService Token 两项均 PASS；不是普通测试环境的 SKIP。
- [Windows 候选 Artifact #9935300650](https://github.com/lifei6671/xtunnel/actions/runs/33868358816/artifacts/9935300650) 名称为 `m8-05-windows-amd64-43808023fd3bff5b19243ac63ecaf9d3789ee40b-attempt-1`，按 CI 配置保留 14 天。包含 Server EXE、Manifest、产品报告和校验清单四个文件；Server 为 Windows/amd64 PE、Go 1.27.1，版本 `v0.1.0-ci.43808023fd3bff5b19243ac63ecaf9d3789ee40b`。
